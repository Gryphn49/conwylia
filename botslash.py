...
"""
    def __init__(self, name, owner, allies=[], tps=[], union="", uP=""):
        ...
        self.union = union # is union -- Options: blank (not set), False, Senior, Junior
    ...
    def unions(self): # returns whether the nation is in a union or not.
        union = self.union
        if union == "Senior" or union == "Junior": # this does not say whether they are a senior or junior nation cuz imo doesn't really matter outside of rp rules. I could add it, but I could also just add another command to check that.
...
nations = {key: Nation(key, stored[key]["owner"], stored[key]["allies"], stored[key]["tps"]) for key in stored} # creates nation class for each nation from the information provided in stored.
...
async def self(interaction: discord.Interaction, name: str, owner: str):
    ...
    with open(nationFile, "wb") as tf:
        ...
        tf.close()
    await interaction.response.send_message(f"The nation {name} has been added to the database.") # response to user
...
async def self(interaction: discord.Interaction):
    await interaction.response.send_message("The nations currently in the database are: %s.")%", ".join(stored.keys()) # this grabs all of the nations names which are in stored as the keys and joins them together with a comma. Better than the multi-line for loop I used to have do it...
...
async def self(interaction: discord.Interaction, name: str):
    try: # this tries to delete the nation 
        ...
        with open(nationFile, "wb") as tf:
            pickle.dump(stored,tf)
            tf.close()
        await interaction.response.send_message(f"The nation {name} has been deleted from the database.") # response
    except KeyError:
...
async def self(interaction: discord.Interaction, nation_name: str, allied_nation_name: str): # nation name = "asking nation", whichever nation is initiating the alliance (ik its mutual but eh),   allied nation name = "to be allied nation", the other nation :)
